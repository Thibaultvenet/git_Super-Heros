...
def recherche_avancee(data, filtres):
    """
    Effectue une recherche avancée dans les données JSON en utilisant plusieurs filtres.
    :param data: Liste de dictionnaires représentant les données JSON.
    :param filtres: Dictionnaire des filtres à appliquer (clé: nom de l'attribut, valeur: valeur recherchée).
    :return: Liste des éléments correspondant aux filtres.
    """
    resultats = []
    for element in data:
        correspond = True
        for cle, valeur in filtres.items():
            if cle in element and str(valeur).lower() not in str(element[cle]).lower():
                correspond = False
                break
        if correspond:
